...
start_cheeze() {
	...
	ssh root@pt1 "cd /home/kukania/Koofs_proj/FlashFTLDriver/; export MS_TIME_SL=$udelay; chrt --rr 99 ./cheeze_block_driver > /$(cat /etc/hostname)/$output_file_kukania 2>&1 < /dev/null" &
...
setup_log() {
	...
	output_file_kukania="${output_dir_org_kukania}/${alias}"
